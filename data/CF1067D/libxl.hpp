...
namespace lxl
{
    ...
    double mk_double(int decimal_place)
    {
        const auto rng = mk_rand<unsigned long long>(0, std::pow<unsigned long long>(10, decimal_place));
        const double res = (double)rng / (double)std::pow<unsigned long long>(10, decimal_place);
        return res;
    }
