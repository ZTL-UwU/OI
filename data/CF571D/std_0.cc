...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
        if (s1.siz[i])
            s1.adde(s1.tot, s1.bel[i]);
    s2.tot++;
    for (int i = 1; i <= n; i++)
        if (s2.siz[i])
            s2.adde(s2.tot, s2.bel[i]);
    s1.dfs(s1.tot);
    s2.dfs(s2.tot);
    st1.build(1, 1, s1.tot);
    st2.build(1, 1, s2.tot);
    for (int i = 1; i <= qu; i++)
    {
        if (q[i].opt == 4)
            st2.modify(1, s2.dfn[q[i].x], s2.dfn[q[i].x] + s2.sz[q[i].x] - 1, i);
        if (q[i].opt == 5)
        {
    ...
    {
        if (q[i].opt == 3)
            st1.modify(1, s1.dfn[q[i].x], s1.dfn[q[i].x] + s1.sz[q[i].x] - 1, q[i].y);
        if (q[i].opt == 4)
        {
