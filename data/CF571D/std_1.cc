...
struct qwq
{
    ...
    void ae(int u, int v) { edge[cnt].next = head[u], edge[cnt].to = v, head[u] = cnt++; }
    ...
    void dfs(int x)
    {
        ...
        for (int i = head[x]; i != -1; i = edge[i].next)
            dfs(edge[i].to), sz[x] += sz[edge[i].to];
    }
    void ini()
    {
        tot = n;
...
int main()
{
    ...
    for (int i = 1; i <= m; i++)
    {
        ...
        scanf("%s", s);
        if (s[0] == 'U')
            q[i].tp = 1, scanf("%d%d", &q[i].b, &q[i].a), cam.merge1(q[i].a, q[i].b);
        if (s[0] == 'M')
            q[i].tp = 2, scanf("%d%d", &q[i].b, &q[i].a), pol.merge1(q[i].a, q[i].b);
        if (s[0] == 'A')
            q[i].tp = 3, scanf("%d", &q[i].a);
        if (s[0] == 'Z')
            q[i].tp = 4, scanf("%d", &q[i].a);
        if (s[0] == 'Q')
            q[i].tp = 5, scanf("%d", &q[i].a);
    }
    cam.tot++, pol.tot++;
    for (int i = 1; i <= n; i++)
    {
        if (cam.bsz[i])
            cam.ae(cam.tot, cam.bel[i]);
        if (pol.bsz[i])
            pol.ae(pol.tot, pol.bel[i]);
    }
    ...
    {
        if (q[i].tp == 2)
            pol.merge2(q[i].a, q[i].b);
        if (q[i].tp == 4)
            s2.cg(1, 1, lim2, pol.dfn[pol.bel[q[i].a]], pol.dfn[pol.bel[q[i].a]] + pol.sz[pol.bel[q[i].a]] - 1, i);
        if (q[i].tp == 5)
        {
    ...
    {
        if (q[i].tp == 1)
            cam.merge2(q[i].a, q[i].b);
        if (q[i].tp == 3)
            s1.add(1, 1, lim1, cam.dfn[cam.bel[q[i].a]], cam.dfn[cam.bel[q[i].a]] + cam.sz[cam.bel[q[i].a]] - 1,
                   cam.bsz[q[i].a]);
        if (q[i].tp == 4)
            for (int j = 0; j < q[i].v.size(); j++)
                res[q[i].v[j]] -= s1.query(1, 1, lim1, cam.dfn[q[q[i].v[j]].a]);
        if (q[i].tp == 5)
            res[i] += s1.query(1, 1, lim1, cam.dfn[q[i].a]), printf("%lld\n", res[i]);
    }
    return 0;
}