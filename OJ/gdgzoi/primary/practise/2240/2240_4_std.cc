...
#define v e[i].to
    void dfs1(int x, int prev, int d, int deep)
    {
        ...
        for (int i = head[x]; i; i = e[i].nxt)
        {
            if (v != fa[x])
            {
                dfs1(v, x, d + e[i].w, deep + 1);
                size[x] += size[v];
                son[x] = size[v] > size[son[x]] ? v : son[x];
            }
        }
    }
...
#undef v
    ...
    void modify(int x)
    {
        ...
        col[x] ^= 1;
        if (col[x])
            del(dis[x]), dis[x].del(0), add(dis[x]);
        else
            del(dis[x]), dis[x].add(0), add(dis[x]);
