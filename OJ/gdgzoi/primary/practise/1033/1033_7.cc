...
public:
    ...
    friend std::istream &operator>>(std::istream &is, interger<Size> &value)
    {
        std::string str;
        is >> str;

        const interger<Size> tmp = interger<Size>(str);
        value = tmp;

        return is;
    }

    friend std::ostream &operator<<(std::ostream &os, interger<Size> &value)
    {
        os << value.to_string();
        return os;
    }
...
int main()
{
    interger<5000> a, b, c, d;
    std::cin >> a >> b;

    const interger<5000> tmp1 = interger<5000>(a / b);
    c = tmp1;
    const interger<5000> tmp2 = interger<5000>(a % b);
    d = tmp2;
