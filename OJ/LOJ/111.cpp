...
int main()
{
    ...
    for (int w = 1; w < n; w <<= 1)
    {
        ...
        for (i = 1; i <= n; i++) ++bsk[rnk[(tmp[i] = suf[i]) + w]];
        for (i = 1; i <= m; i++) bsk[i] += bsk[i - 1];
        for (i = n; i >= 1; i--) suf[bsk[rnk[tmp[i] + w]]--] = tmp[i];
        memset(bsk, 0, sizeof(bsk));
        for (i = 1; i <= n; i++) ++bsk[rnk[tmp[i] = suf[i]]];
        for (i = 1; i <= m; i++) bsk[i] += bsk[i - 1];
        for (i = n; i >= 1; i--) suf[bsk[rnk[tmp[i]]]--] = tmp[i];
